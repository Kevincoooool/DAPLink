...
error_t target_opt_init(void)
{
	...
	if (0 == swd_flash_syscall_exec(&STM32_ALGO[Select_algo].algo.sys_call_s, STM32_ALGO[Select_algo].algo.init, 0, 0, 0, 0, 0))
	{
		return ERROR_INIT;
	}
...
{
	if (0 == swd_flash_syscall_exec(&STM32_ALGO[Select_algo].algo.sys_call_s, STM32_ALGO[Select_algo].algo.uninit, 0, 0, 0, 0, 0))
	{
		return ERROR_INIT;
	}
	return ERROR_SUCCESS;
...
{
	if (0 == swd_flash_syscall_exec(&STM32_ALGO[Select_algo].algo.sys_call_s, STM32_ALGO[Select_algo].algo.erase_sector, addr, 0, 0, 0, 0))
	{
...
error_t target_opt_erase_chip(void)
{
	...
	if (0 == swd_flash_syscall_exec(&STM32_ALGO[Select_algo].algo.sys_call_s, STM32_ALGO[Select_algo].algo.erase_chip, 0, 0, 0, 0, 0))
	{
