...
uint8_t W25QXX_ReadSR(void)
{
	uint8_t byte = 0;
	CS_L();			  //使能器件
	SPI_W25Q_RW(W25X_ReadStatusReg); //发送读取状态寄存器命令
	byte = SPI_W25Q_RW(0Xff);		  //读取一个字节
	CS_H();				  //取消片选
	return byte;
}
//写W25QXX状态寄存器
...
{
	CS_L();			   //使能器件
	SPI_W25Q_RW(W25X_WriteStatusReg); //发送写取状态寄存器命令
	SPI_W25Q_RW(sr);				   //写入一个字节
	CS_H();				   //取消片选
}
//W25QXX写使能
//将WEL置位
...
{
	CS_L();			//使能器件
	SPI_W25Q_RW(W25X_WriteEnable); //发送写使能
	CS_H();				//取消片选
}
//W25QXX写禁止
//将WEL清零
...
{
	CS_L();			 //使能器件
	SPI_W25Q_RW(W25X_WriteDisable); //发送写禁止指令
	CS_H();				 //取消片选
}
//读取芯片ID
//返回值如下:
...
void W25QXX_Read(uint8_t *pBuffer, uint32_t ReadAddr, uint16_t NumByteToRead)
{
	uint16_t i;
	CS_L();					   //使能器件
	SPI_W25Q_RW(W25X_ReadData);			   //发送读取命令
	SPI_W25Q_RW((uint8_t)((ReadAddr) >> 16)); //发送24bit地址
	SPI_W25Q_RW((uint8_t)((ReadAddr) >> 8));
	SPI_W25Q_RW((uint8_t)ReadAddr);
	for (i = 0; i < NumByteToRead; i++)
	{
		pBuffer[i] = SPI_W25Q_RW(0XFF); //循环读数
	}
	CS_H();
}
...
void W25QXX_Write_Page(uint8_t *pBuffer, uint32_t WriteAddr, uint16_t NumByteToWrite)
{
	...
	CS_L();						//使能器件
	SPI_W25Q_RW(W25X_PageProgram);				//发送写页命令
	SPI_W25Q_RW((uint8_t)((WriteAddr) >> 16)); //发送24bit地址
	SPI_W25Q_RW((uint8_t)((WriteAddr) >> 8));
	SPI_W25Q_RW((uint8_t)WriteAddr);
	for (i = 0; i < NumByteToWrite; i++)
		SPI_W25Q_RW(pBuffer[i]); //循环写数
	CS_H();			  //取消片选
...
void W25QXX_Erase_Chip(void)
{
	...
	W25QXX_Wait_Busy();
	CS_L();		  //使能器件
	SPI_W25Q_RW(W25X_ChipErase); //发送片擦除命令
	CS_H();			  //取消片选
...
void W25QXX_Erase_Sector(uint32_t Dst_Addr)
{
	...
	W25QXX_Wait_Busy();
	CS_L();					   //使能器件
	SPI_W25Q_RW(W25X_SectorErase);			   //发送扇区擦除指令
	SPI_W25Q_RW((uint8_t)((Dst_Addr) >> 16)); //发送24bit地址
...
{
	CS_L();		  //使能器件
	SPI_W25Q_RW(W25X_PowerDown); //发送掉电命令
	CS_H();			  //取消片选
							  //	delay_us(3);			//等待TPD
}
//唤醒
